...
export default async function TransparenciaSectionPage({
  params,
}: {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-slate-700">
          Sección no encontrada. <Link href="/transparencia" className="text-blue-600 hover:underline">Volver</Link>
        </div>
      </div>
    )
  }
        ...
        <section className="bg-white rounded-xl border p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {transparenciaSections
              ...
              .map((s) => (
                <Link key={s.slug} href={`/transparencia/${s.slug}`} className="text-sm px-3 py-1 border rounded hover:bg-slate-50">
                  {s.id}. {s.title}
                </Link>
              ))}
          </div>
          <Link href="/transparencia" className="inline-block mt-4 text-blue-600 hover:underline text-sm">
            ← Volver al índice
          </Link>
        </section>
      </main>
